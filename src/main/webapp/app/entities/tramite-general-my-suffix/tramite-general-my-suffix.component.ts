...
@Component({
    selector: 'jhi-tramite-general-my-suffix',
    templateUrl: './tramite-general-my-suffix.component.html'
})
export class TramiteGeneralMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.tramiteGeneralService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<TramiteGeneralMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/tramite-general-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/tramite-general-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: TramiteGeneralMySuffix) {
        return item.id;
    }
