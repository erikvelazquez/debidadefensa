...
@Component({
    selector: 'jhi-tramite-migratorio-my-suffix',
    templateUrl: './tramite-migratorio-my-suffix.component.html'
})
export class TramiteMigratorioMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.tramiteMigratorioService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<TramiteMigratorioMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/tramite-migratorio-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/tramite-migratorio-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: TramiteMigratorioMySuffix) {
        return item.id;
    }
